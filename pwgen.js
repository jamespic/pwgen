...
function PwGen($scope) {
  var salt = CryptoJS.enc.Hex.parse('0000000000000000'); // Hard-coded - only way to make this repeatable
  ...
  $scope.generate = function() {
    if ($scope.pw1 !== $scope.pw2) {
      ...
      var key = CryptoJS.PBKDF2($scope.userName + ":" + $scope.siteName + ":" + $scope.pw1, salt, {keySize: 128/32, iterations: $scope.iterations});
